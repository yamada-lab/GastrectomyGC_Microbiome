...
#colors = ['#0072B2','#F0E442','r','g','m','c','k','w']
#colors = ['#F0E442','#0072B2','r','g','m','c','k','w']
#two factors
colors = ['#F7941D','#0072BC','#F0E442','g','m','c','k','w']
#three factors
#colors = ['#0072BC','#F7941D','#F0E442','g','m','c','k','w']
#define the font
...
def read_data(input_file,output_file,otu_only):
    with open(input_file, 'r') as inp:
        if not otu_only:
            rows = [line.strip().split()[:-1] for line in inp.readlines() if len(line.strip().split())>4]
        else:
            rows = [line.strip().split()[:-1] for line in inp.readlines() if len(line.strip().split())>4 and len(line.strip().split()[0].split('.'))==9] # a feature with length 8 will have an OTU id associated with it
    classes = list(set([v[2] for v in rows if len(v)>3]))
    print len(rows)
    if len(classes) < 1: 
...
def plot_histo_ver(path,params,data,report_features):
    ...
    ax.set_ylabel("LDA SCORE (log 10)", fontweight='bold')
    ax.yaxis.grid(True) 
    a,b = ax.get_xlim()
