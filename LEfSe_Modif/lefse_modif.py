...
### corrections FDR 
def FDRcorrection(pvaldict,p,q):
	from rpy2.robjects.packages import importr
	from rpy2.robjects.vectors import FloatVector
	stats 		= importr('stats')
	feat_namel 	= list(pvaldict.keys())
	pval_tot 	= list(pvaldict.values())
	kw_ok1 		= ([z[0] for z in pval_tot])
	pval 		= ([z[1] for z in pval_tot])
	p_adjust1 	= stats.p_adjust(FloatVector(pval), method = 'BH')
	p_adjust 	= list(p_adjust1)
	kw_ok 		= [x<= q for x in p_adjust]
	Feat_correct={}
	for a,b,c,d,e in itertools.izip(feat_namel,kw_ok1,pval,kw_ok,p_adjust):
		Feat_correct[a]=[b,c,d,e]
	return Feat_correct


def save_res_modif(res,filename): 
	with open(filename, 'w') as out:
		#cls means is the means for each group
		for k,v in res['cls_means'].items():
			out.write(k+"\t"+str(math.log(max(max(v),1.0),10.0))+"\t")
			if k in res['lda_res_th']:
				for i,vv in enumerate(v):
				#writing where the enrichement is
					if vv == max(v):
						out.write(str(res['cls_means_kord'][i])+"\t")
						break
				out.write(str(res['lda_res'][k])) 
			else: out.write("\t")
			out.write( "\t" + (res['wilcox_resval'][k] if 'wilcox_resval' in res and k in res['wilcox_resval'] else "-"))
			out.write( "\t" + (res['wilcox_resadj'][k] if 'wilcox_resadj' in res and k in res['wilcox_resadj'] else "-")+"\n")
