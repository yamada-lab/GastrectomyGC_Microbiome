...
#colors = ['#F0E442','#0072B2','b','m','c',[1.0,0.5,0.0],[0.0,1.0,0.0],[0.33,0.125,0.0],[0.75,0.75,0.75],'k']
#colors = ['#08FF00','#FF3300','b','m','c',[1.0,0.5,0.0],[0.0,1.0,0.0],[0.33,0.125,0.0],[0.75,0.75,0.75],'k']
colors = ['#F7941D','#0072BC','#F0E442','g','m','c','k','w']
...
def read_data(input_file,params):
	...
	#rows is name and statistical test results
	all_names = [lin[0] for lin in rows]
	to_add = []

	abundances = [float(v) for v in zip(*rows)[1] if v >= 0.0]
	tree = {}
	tree['classes'] = list(set([v[2] for v in rows if len(v)>3])) #determine which had the significant
	tree['classes'].sort()
	...
	for i,v in enumerate(rows):
		if len(v)>3:
			if len(cls2) > 0: all_nodes[i].set_color(colors[cls2.index(v[2])%len(colors)])
				...
				if v[2].count('rgbcol') > 0: #determine the stage
...
def plot_points(father,params,pt_scale,ax):
	children = father.get_children()
	children.sort(key = lambda a: -int(a.get_color() == '#ffffff')*a.abundance)
	...
	pw = params['markeredgewidth'] if col == '#ffffff' else params['markeredgewidth']*3.0
...
def plot_lines(father,params,depth,ax,xf):
	...
	for i,child in enumerate(children):
		...
		if len(father.name) >= depth-params['radial_start_lev']: 
			...
			if father.get_color() != '#ffffff' and father.get_color() == child.get_color() and params['colored_connectors']:
				col = child.get_color()
				lw *=2.5
...
def plot_names(father,params,depth,ax,u_i,seps):
	...
	if len(children)==0:
        ...
        for i,child in enumerate(children):
                ...
                if i == 0: fr_0 = fr
		fr_1 = to 
        if father.get_color() != '#ffffff' and params['labeled_start_lev'] < l <= params['labeled_stop_lev']+1:
                col = father.get_color()
